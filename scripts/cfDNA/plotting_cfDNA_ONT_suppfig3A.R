...
minCN = -1
maxCN = 1
# Align all cfDNA samples to ONT B4 sample
for (sample in c("B4", "B67", "B169")){
  cov_ILL <-
    read.table(
      paste0("../../../../bladder_cancer/liquid_biopsy/bam_files/", sample, "/ichorCNA_", sample, ".cna.seg"),
      header = TRUE,
      sep = "\t"
    )
  colnames(cov_ILL) <-
  ...
  meds <- median(cov_ILL$logR, na.rm = TRUE)
  if (meds < 0) {
    cov_ILL$logR <- cov_ILL$logR + abs(meds)
  } else {
    cov_ILL$logR <- cov_ILL$logR - meds
  }
  ...
  cov_ONT <-
    read.table(
      "../../../../bladder_cancer/ont/cnvs/coverage/B4tumor.cov.gz",
      header = TRUE,
      sep = "\t"
    )
  ...
  for (chrlabel in unique(cov_ONT$chr)) {
    ...
    p_ill <- ggplot(df_ill, aes(x = pos, y = signal, color = event)) +
      ...
      scale_y_continuous(labels=NULL, breaks = seq(-1, 1, by = 0.25), limits = c(-1, 1)) +
      xlab(chrlabel) +
      scienceTheme +
      theme(legend.position = "none")
    
    # Stack ONT and Illumina vertically
  ...
  ggsave(file=paste0("supp_fig3A_", sample, "_ONT_cfDNA.png"), width=40, height=12, limitsize = FALSE)
} 
